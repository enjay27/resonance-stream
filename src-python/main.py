...
# Force UTF-8 for stable pipe communication across different OS locales
sys.stdin = io.TextIOWrapper(sys.stdin.buffer, encoding='utf-8')
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

# --- CONFIGURATION & LIMITS ---
# Prevents memory exhaustion (OOM) on 6GB VRAM cards (GTX 1060/2060)
MAX_CHAR_INPUT = 2000
...
class TranslationManager:
    ...
    def load_dictionary(self):
        if os.path.exists(self.dict_path):
            try:
                ...
                # Filter out standard brackets to prevent regex breakage
    ...
    def preprocess_chunking(self, text):
        original_input = text
        split_marker, current_text = "||SPLIT||", text
        protected_map, protected_count = {}, 0

        # Dictionary & Recruitment Shielding
        all_targets = [(ja, ko) for ja, ko in self.custom_dict.items() if ja in text]
        recruit_pattern = r'@[A-Za-z0-9]+(?:[\s]+[A-Za-z0-9]+)*'
        for m in set(re.findall(recruit_pattern, text)): all_targets.append((m, m))
        all_targets.sort(key=lambda x: len(x[0]), reverse=True)

        for ja, val in all_targets:
...
def main():
    ...
    # Tier Calibration
    # Low: Greedy search (beam=1) for speed on GTX 1060
    # High: Wide search (beam=10) for maximum accuracy on RTX 3060+
    tier_cfg = {
    ...
    try:
        # Hardware Detection & Optimized Precision
        cuda_count = ctranslate2.get_cuda_device_count()
        if cuda_count > 0:
            device, compute_type = "cuda", "int8_float16" # Balanced memory/speed for RTX cards
        else:
            device, compute_type = "cpu", "int8"

        # Translator Init with Thread Control
        ...
        while True:
            ...
            try:
                ...
                # Safety: Character limit check
                if len(input_text) > MAX_CHAR_INPUT:
                    manager.log_error(f"Input too long ({len(input_text)}).")
                    continue

                chunks_data, original_input = manager.preprocess_chunking(input_text)
                ...
                for idx, (chunk_text, is_protected) in enumerate(chunks_data):
                    if is_protected:
                        ...
                        # Stable Batching Parameters
                        res = translator.translate_batch(
                            [tokens], target_prefix=[["kor_Hang"]],
                            beam_size=cfg["beam"], patience=cfg["patience"],
                            repetition_penalty=cfg["rep_pen"],
                            max_batch_size=1, batch_type="tokens"
                        )
                ...
                # Build Result
                result_body = {"type": "result", "pid": pid, "translated": final_output}
                if args.debug:
                    result_body["diagnostics"] = [
                ...
                print(json.dumps(result_body, ensure_ascii=False), flush=True)

                # Manual GC to keep 6GB cards stable
