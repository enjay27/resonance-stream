...
pub fn launch_ai_server(app: &AppHandle, model_path: &PathBuf, config: &crate::config::AppConfig) -> Option<Child> {
    ...
    match server_cmd.spawn() {
        ...
        Err(e) => {
            ...
            super::emit_translator_state(app, "Error", &format!("Failed to start llama-server.exe. ({})", e));
            None
        }
    }
}
